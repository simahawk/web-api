...
def migrate(env, version):
    ...
    `webservice.url_param` records, now that `url_param_ids` (available on
    both backends and endpoints) is the dedicated mechanism for that.
    ...
    for backend in backends:
        ...
        env["webservice.url_param"].create(
            [
                {
        ...
        _logger.info(
            "webservice.backend(%s): moved querystring %r to url_param_ids",
            backend.id,
            parts.query,
        )
